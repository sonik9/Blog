...
-- Дата скрипта: 28/08/2015 6:31:42 PM
...
CREATE TABLE blg_post_categories (
  pst_cat_id int(11) NOT NULL AUTO_INCREMENT,
  pst_cat_name varchar(50) NOT NULL,
  pst_cat_enable tinyint(1) NOT NULL DEFAULT 1,
  pst_cat_pstcount int(11) NOT NULL DEFAULT 0,
  PRIMARY KEY (pst_cat_id),
  UNIQUE INDEX pst_cat_name (pst_cat_name)
)
ENGINE = INNODB
AUTO_INCREMENT = 4
AVG_ROW_LENGTH = 5461
...
CREATE TABLE blg_post (
  ...
  UNIQUE INDEX UK_blg_post_usr_id (usr_id),
  CONSTRAINT FK_blg_post_blg_post_categories_pst_cat_id FOREIGN KEY (pst_cat_id)
  REFERENCES blg_post_categories (pst_cat_id) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT FK_blg_post_blg_user_usr_id FOREIGN KEY (usr_id)
  REFERENCES blg_user (usr_id) ON DELETE RESTRICT ON UPDATE RESTRICT
)
ENGINE = INNODB
AUTO_INCREMENT = 10
